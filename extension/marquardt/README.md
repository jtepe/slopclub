...
It receives every parsed segment joined into one complete command chain, or
the original command when parsing failed. A **non-critical** verdict allows
the command immediately. A **critical** verdict displays the judge's
explanation and leaves the final decision to you. A failed or malformed judge
call remains at review and can be retried.
